...
- [x] Functions are used to organize code
...
- [x] Code is appropriately formatted to show flow of control as well as in well indent
- [ ] Code use parameters to combine functions together
- [x] Comments do not contain blocks of old code that is no longer in use
<br>
Before the qualifier, we were expecting that most functions will have its "opposite" one, for example, if a function can move the robot forward, then the opposite one could make it backward - so to shorten the program, we could have adopted this style: 
...
### The program itself
I, axmmisaka, as a ameteur programmer who has made a little improvements on Github for some opensource projects, deeply know how unreliable these home-make projects are. To improve reliability, the program itself should not crash at first. Below are some points.<br>
- Pointers in C Programming Language is powerful, but sometimes they may be fatal, like dangling or wild pointers. Our program does not need complicated data structures, as a result, we used no pointers because they are no longer needed. 
- Goto statement may be useful, but it makes the program more likely to crash, said Dijkstra. We used goto statement in national qualifier to reduce our workload, but now we successfully converted them into loops and break or continue statement.
- Global variables may be a must, but we reduced its number to minimum. Too many global variables can make the program less realible because the cohesion of the program will decrease, and it may have a negative effect on other local variables.
- No undefined behaviors. The compiler provided by KIPR is pretty reliable, but like every compiler, it is unable to do anything with undefined behaviors. Thus, for some statements like `printf("%d, %d",++n,frontSensorDetect(n,2));` or some woodenheaded statement like `msleep(++time+++++++time++);` are strictly prohibited in our coding group.
...
# Maintainability
As we use git as the version control system, it is very easy to see where the program has changed. Although we do not commit changes too often because most of the changes are specific values that need to be measured every time, we commit when major changes took place. Thus, if anything goes wrong, it will be very easy for us to roll back to previous version or use the diff tool to see where these problems may occur.<br>
...
Some people's English is not that proficient, and they update github in Chinese.<br>

Some codes are initially written by teammates who participate in Olympiad in Informatics (like USACO), they write super-fast, but variables are named poorly, no comments, and there's even no indentation! If any of us see such situation, we will immediately go ask him(before he forget anything) the meaning of these variable names(most are abbreviations of Chinese Pinyin or simple English words), and change the code to the most readable one. For example, in commit e8e3e49ca3d51b7308c69d09288a25902928bf0b, we changed an nearly unreadable code into a readable one without changing its core algorithm, below is the difference of this commit and previous one using diff tool:
...
After several reviews and changes, the code is able to finish all assigned tasks effectively.
